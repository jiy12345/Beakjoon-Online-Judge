...
- 문제 상황
 같은 알파벳이 적힌 칸을 두번 지날 수 없다고 할 때, 죄측 상단에서 시작해서 말이 최대로 갈 수 있는 칸이 몇 칸인지를 구하는 프로그램을 작성하라!

 첫 위치 또한 경로에 포함된다고 본다!

2. 풀이 계획
 DFS를 진행하되, 비트마스킹을 통해 어떤 알파벳이 있는지 계산을 계속해서 진행해야 할듯 하다!
